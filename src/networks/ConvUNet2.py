...
class UNet3DWithSamplePointsAhmed(UNet3DWithSamplePoints):
    ...
    def data_dict_to_input(self, data_dict):
        x_in = data_dict["centroids"][0].unsqueeze(0)  # (1, n_in, 3)
        x_out = data_dict[
            "df_query_points"
        ]  # (1, 3, n_x, n_y, n_z)  # (n_x, n_y, n_z, 3)
        df = data_dict["df"].unsqueeze(0)  # (1, 1, n_x, n_y, n_z)

        vel = torch.tensor([data_dict["info"][0]["velocity"]]).view(
            -1,
        )
        vel = vel * torch.ones_like(df)

        input_grid_features = torch.cat(
            (df, vel, x_out), dim=1
        )  # (1, 5, n_x, n_y, n_z)

        input_grid_features, x_in = (
            input_grid_features.to(self.device),
            x_in.to(self.device),
        )
    ...
    def eval_dict(self, data_dict, loss_fn=None, decode_fn=None, **kwargs):
        input_grid_features, output_points = self.data_dict_to_input(data_dict)
        output_points = output_points[:, :: self.subsample_eval]
    ...
    def loss_dict(self, data_dict, loss_fn=None):
        ...
        pred = self(input_grid_features, output_points)#torch.Size([1, 52409, 1])
        if loss_fn is None:
            loss_fn = self.loss
        return {
            "loss": loss_fn(
                pred.view(1, -1),
                data_dict["pressure"][0][:: self.subsample_train]
                .view(1, -1)
                .to(self.device),
            )
        }
